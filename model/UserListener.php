...
class model_UserListener extends MachII_framework_Listener {
	...
	function logIn(&$event){	
		...
		$login = $event->getArg('login');
		$password = $event->getArg('password');
		 
		$objUserDao = new UserDao();
		$objUserBean = $objUserDao->getByEmailAndPassword($login,$password);
		...
		if($objUserBean->getUserId() != "" && $objUserBean->getStatus() == 1) {
			...
			$arrResult = array(validationResult => false,  
						  errorMessage => "Błędny adres e-mail lub niewłaściwe hasło.",
						  fieldName => "formLoginEmail");
			header("Location: ".$SN."login/formLoginEmail.html");
	...
	function createUserWithActivation(&$event) {
		...
		$name = $event->getArg('email');
  		$fromEmail = "Musikkglede <musikkglede@musikkglede.no>";
		$toEmail = $event->getArg('email');
		$subject  = 'Musikkglede - Activation link';
	...
	function executeForgotPassword(&$event){	
		...
		if ($objUserBean->getUserId()!=""){
			...
			$subject  = "Musikkglede - Forgot Password";
		...
		// tresc wiadomosci
			...
			$messageContent .= "<b>Password: </b>".$objUserBean->getPassword()."<br /><br/>";
			$messageContent .= "Click the link below and login to the store:<br/>";
	  		$messageContent .= "<a href=\"http://www.musikkglede.no\">www.musikkglede.no</a><br/><br/>";
	    	...
	    	$mail->setHTML($messageContent);
    		$result = $mail->send(array($toEmail));
			$this->validationResultTrue();
		} else{
			$arrResult = array(validationResult => false,  
						  errorMessage => "Entered address has not been found in database.",
